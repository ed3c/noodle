...
for name in ['wrong-digest','wrong-revision','admitted-ledger','valid-proposal','live-session','orphan-live-group']:
 with tempfile.TemporaryDirectory(prefix='noodle84-negative-') as scratch:
  ...
  if name=='admitted-ledger':
   ...
   state['effect_ledger'].append({'effect_id':effect,'effect':{'effect_id':effect,'type':'initial_admission','payload':{'order_id':subject,'initial_revision':proposal['initial_revision']},'created_at':'2026-09-17T00:00:00Z'},'status':'done','attempts':1})
  ...
  if name in ['live-session','orphan-live-group']:
   ...
   (folder/'meta.json').write_text(json.dumps({'status':'running','session_id':session}))
  ...
  try:
   ...
   passed=q.returncode!=0 and result.get('status')=='refused' and preserved
