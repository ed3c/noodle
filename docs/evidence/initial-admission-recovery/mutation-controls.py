...
r=Path(__file__).parent;sel=json.loads((r/'execution-selection.json').read_text());source=Path(sel['control_root'])/'.worktrees/noodle-84-0-execute';out=r/'mutation-controls';out.mkdir()
...
cases=[
 ('canonical-schema',evidence,'snapshot.State.SchemaVersion != statever.Current','false','TestAdmissionRecoveryRefusalGates/unknown_schema'),
